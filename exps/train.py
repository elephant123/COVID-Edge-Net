...
class Trainer():
    ...
    def training(self, epoch):
        ...
        for i, (image, target) in enumerate(tbar):
            ...
            if torch_ver == "0.3":
                ...
                T_canny = Variable(T_canny)

            T_canny=torch.tensor(T_canny)
            T_canny = T_canny.view(T_canny.size(0), self.nclass, T_canny.size(1), T_canny.size(2))
            outputs = self.model(image.float(),T_canny)
    ...
    def validation(self, epoch):
        ...
        val_whole_loss.append(val_loss_all / (i + 1))
...
if __name__ == "__main__":
    ...
    print(val_whole_loss.index(min(val_whole_loss)))
    # for v in range(args.start_epoch, args.epochs):
    for v in range(args.start_epoch, 30):
        if v!= val_whole_loss.index(min(val_whole_loss)):
            pre_model = os.path.join(
            '/media/DataHD/wk/covid/wk/DFF_canny_improatt/exps/runs/sbd/dff/dff/','checkpoint_{}.pth.tar'.format(v + 1))
            os.remove(pre_model)
