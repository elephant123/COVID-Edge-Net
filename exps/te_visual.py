...
from option_visual import Options
...
def te(args):
    ...
    if args.eval: # set split='val' for validation set testing
        ...
        #                                    transform=input_transform, crop_size=args.crop_size)
        testset = get_edge_dataset(args.dataset, split='train', mode='vis',
                                   transform=input_transform, crop_size=args.crop_size)
    ...
    if args.eval:
        ...
        outdir = '%s/%s/%s_vis>0.6'%(args.dataset, args.model, args.checkname)
        print(outdir)
    ...
    if args.eval:
        ...
        for i, (images, masks, im_paths, im_sizes) in enumerate(tbar):
            ...
            for img_l in range(len(images)):
                ...
                A = np.float32(cv2.Canny(A, 150, 200) / 255.0)
                T_canny.append(A)
            with torch.no_grad():
                images = [image.unsqueeze(0) for image in images]
                images = torch.cat(images, 0)
                ...
                for predict, impath in zip(fuse_list, im_paths):
                    ...
                    a, b = predict.shape
                    predict = predict.reshape(1, a, b)
                    # predict = predict.view(1, predict.size(0), predict.size(1))
                    visualize_prediction(args.dataset, path, predict)
