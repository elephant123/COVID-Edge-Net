...
from datasets_train import get_edge_dataset, test_batchify_fn
from models import get_edge_model
from visualize import visualize_prediction

from option_test_trainsamples import Options
...
def te(args):
    ...
    if args.eval: # set split='val' for validation set testing
        ...
        testset = get_edge_dataset(args.dataset, split='train', mode='testval',
                                   transform=input_transform, crop_size=args.crop_size)
    ...
    if args.eval:
        ...
        for i in range(testset.num_class):
            outdir_side5 = '%s/%s/%s_train/side5/class_%03d'%(args.dataset, args.model, args.checkname, i+1)
            print(outdir_side5)
            if not os.path.exists(outdir_side5):
                os.makedirs(outdir_side5)
            outdir_list_side5.append(outdir_side5)

            outdir_fuse = '%s/%s/%s_train/fuse/class_%03d'%(args.dataset, args.model, args.checkname, i+1)
            print(outdir_fuse)
    ...
    if args.eval:
        for i, (images, im_paths, im_sizes) in enumerate(tbar):
            T_canny = []
            for img_l in range(len(images)):
                A = images[img_l]
                A = np.uint8(np.array(A))
                A = np.transpose(A, (1, 2, 0))
                A = np.float32(cv2.Canny(A, 100, 200) / 255.0)
                T_canny.append(A)
            ...
            with torch.no_grad():
                ...
                T_canny = torch.tensor(T_canny)
                T_canny = T_canny.view(T_canny.size(0), 1, T_canny.size(1), T_canny.size(2))
                outputs = model(images.float(), T_canny)
                ...
                for predict, impath in zip(side5_list, im_paths):
                        ...
                        # predict_c = predict[i]
                        path = os.path.join(outdir_list_side5[0], impath)
                            # predict_c=predict[i]*255.0
                            # predict_c = predict_c.astype(np.uint8)
                        io.imsave(path, (predict*255).astype(np.uint8))

                for predict, impath in zip(fuse_list, im_paths):


                        path = os.path.join(outdir_list_fuse[0], impath)
                            # predict_c = predict[i] * 255.0
                            # predict_c = predict_c.astype(np.uint8)
                        io.imsave(path, (predict*255).astype(np.uint8))
                # for predict, impath in zip(side5_list, im_paths):
...
def eval_model(args):
    ...
    print(os.path.splitext(args.resume_dir))
