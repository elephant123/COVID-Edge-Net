...
class SBDEdgeDetection(BaseDataset):
    NUM_CLASS = 1
    def __init__(self, root='/media/DataHD/wk/covid/wk/DFF_canny_improatt/data/sbd-preprocess/data_proc', split='train',
...
def _get_sbd_pairs(folder, split='train'):
    ...
    if split == 'train':
        split_f = os.path.join(folder, 'train_ori.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'val':
        split_f = os.path.join(folder, 'test_cls_orig.txt')
        img_paths, mask_paths = get_path_pairs(folder, split_f)
    elif split == 'test':
